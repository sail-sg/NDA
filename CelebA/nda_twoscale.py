...
def parse_args():
    ...
    parser.add_argument(
      "--alpha", 
       type=float, 
       default=0.5,
       help="Weight used to fuse original and downsampled patch distances (0.0 = only downsampled, 1.0 = only original)"
    )
    ...
    parser.add_argument(
       ...
       type=int,
       default=11,
       help="Patch size for projection-based patch conv (used in downsampled matching)."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e3,
        help="Mask value added at invalid patch locations for matching."
    )
    ...
    parser.add_argument("--proj_dim", type=int, default=None,
        help="Projection size for down/projection branch")
...
def main():
    ...
    ### dataset
    import pandas as pd
    df = pd.read_csv('/home/aiops/zhaoyt/diffusion/D-TRAK/codes/celeba/data/celeba/list_eval_partition.txt', delimiter=" ", 
                     names=['path', 'split'],
                     header=None, skipinitialspace=True)
    df['path'] = df.apply(lambda x: "/home/aiops/zhaoyt/diffusion/D-TRAK/codes/celeba/data/celeba/img_align_celeba/{}".format(x['path']), axis=1)
    # print(df.head())
    ...
    if args.gen_source == "val":
        ...
        with open(idx_val_path, 'rb') as handle:
            val_sub_idx = pickle.load(handle)
        print(val_sub_idx[0:5])
                
        gen_dataset = DatasetDict({
            "train": Dataset.from_dict({
                "img": df.loc[val_sub_idx]['path'].tolist(),
            }).cast_column("img", Image())
            ,})
    ...
    # Always apply to train_dataset
    def transform_train_images(examples):
       images = [train_augmentations(image.convert("RGB")) for image in examples["img"]]
       return {"input": images}

    train_dataset.set_transform(transform_train_images)
    ...
    y_full = torch.cat(all_images, 0).to(device, dtype)
        ...
        for i in range(s):
            for j in range(s):
                ...
                for di in range(ratio):
                    ...
                    #    P[row_idx, col_idx] = 1.0 / (ratio * ratio)
    ...
    def patch_score_model_helper_conv_softmax(
        ...
        for start in range(0, B_x * H * W, kernel_batch_size):
            ...
            sim_map = conv_out.permute(1, 0, 2, 3)

            x_norm_batch = (kernels_batch ** 2).sum(dim=(1, 2, 3))
            sq_dist = (
              x_norm_batch.view(bp, 1, 1, 1) +         # broadcast
              y_norm.view(1, N, H_out, W_out) -
              2 * sim_map          # (bP,N,H_y,W_y)
            ).clamp(min=0)
            ...
            if topk is not None:
               ...
               topk_val, _ = torch.topk(weight_flat, topk, dim=2)       # top-k
    ...
    def chunked_y_norm_proj_N(y_eff: torch.Tensor,
        ...
        for n0 in range(0, N, chunk_N):
            ...
            y_unfold = F.unfold(y_chunk,
                            kernel_size=(kH, kW),
                            stride=stride,
                            dilation=dilation)       # [Nc, C*k*k, H_out*W_out]
            Nc = y_unfold.size(0)
            k2 = kH * kW
            # [Nc, C, k2, L] -> [Nc, L, C, k2]
    ...
    def patch_score_model_helper_conv_softmax_down(
        x_patches, y_full, s, std,
        patch_size=(7, 7), stride=(1, 1), dilation=(1, 1),
        mask_value=1e3, kernel_batch_size=16, topk=None, dtype=torch.float32, proj_dim=None):
    ...
    #    W_out = (W_y + 2 * pad_w - (kW - 1) * dilation[1]) // stride[1]
        ...
        H_out, W_out = y_norm_proj.shape[-2:]
        ...
        for start in range(0, B_x * H * W, kernel_batch_size):
            ...
            kernels_batch = x_kernels[start:end]
        #    print("kernels_batch.shape:", kernels_batch.shape)
    ...
    for gen_idx, gen_example in enumerate(tqdm(gen_dataset)):
        ...
        for t_idx, t in enumerate(selected_timesteps):
            ...
            patch_size_down = (args.patch_size_down, args.patch_size_down)
            ...
            # weight_sum: [B_x=1, N]
            weight_softmax = patch_score_model_helper_conv_softmax(
                    x_patches, y_full, s, std,
                    patch_size=patch_size,
            ...
            x_patches_down, x_mask_down = extract_patches(
                   noisy_x,
                   patch_size=patch_size_down,
                   mask_value=args.mask_value,
                )    
                
            x_patches_down = x_patches_down + x_mask_down
                
            # for the downsampled case
            weight_softmax_down = patch_score_model_helper_conv_softmax_down(
                    x_patches_down, y_full, s, std,
                    patch_size=(args.patch_size_down, args.patch_size_down),
                    kernel_batch_size=args.kernel_batch_size,
                    dtype=dtype,
                    topk = args.weight_topk,
                    mask_value=args.mask_value,
            ...
            # Average:
            weight_sum = args.alpha * weight_softmax + (1 - args.alpha) * weight_softmax_down
    ...
    def visualize_topk_per_timestep(score_tensor, gen_dataset, train_dataset, save_dir, topk=10, timesteps=None):
        ...
        for t_idx in range(num_timesteps):
            ...
            #    topk_idx = torch.argsort(scores[i])[:topk]
                topk_idx = torch.argsort(scores[i], descending=True)[:topk]
                plot_images = [gen_dataset[i]['input']]
            #    plot_images.extend([train_dataset[index_map[int(idx)]]['input'] for idx in topk_idx])
