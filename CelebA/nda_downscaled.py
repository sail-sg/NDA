...
def parse_args():
    ...
    parser.add_argument(
       ...
       default="gen",
       choices=["gen", "val"],
       help="If 'val', load images by val indices; else use image folder + gen_index_path.",
    )
...
def main():
    ...
    if args.gen_source == "val":
        ...
        print("Length of val:", len(gen_dataset))    
    else:
        import pandas as pd
        df = pd.DataFrame()
    ...
    y_full = torch.cat(all_images, 0).to(device, dtype)   # [N,C,H,W]
